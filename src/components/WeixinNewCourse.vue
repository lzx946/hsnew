<template>
    <div id="swiper_id" style="background: #CCCCCC;">
        <!--====遮罩层、加载中===-->
        ...
        <!-- 课程 -->
        <div id="item" class="container item-class" style="background: white;">

            <div style="float: left;height: 80%;width: 100%;">
                <img :src="item.url" style="height: 95%;float: left;" />
                <div id="" style="margin-top: 10px; background: #19568a;width: 100%;height: 25%;z-index:-1;">
                    <div id="" class="sing-class">
                    </div>
                    <div id="" style="width: 100%;text-align: center;">
                        <p style="color: white;font-size: 1.2em;overflow: hidden;text-overflow: ellipsis;white-space:nowrap;">
                            <b>{{item.name}}</b>
                        </p>
                        <p style="margin-top: -10px;color: white">第{{period}}期</p>
                    </div>
                    <div id="" v-for="items in item.courses" @click="nowEnter(items.id,items.endtime)">
                        <div style="margin-bottom:5px;margin-top:5px;width: 55%;height: 1px;border: 1px solid #CCCCCC;margin-left: 40%;margin-right: auto;">
                        </div>
                        <div style="margin-left: 40%;background: #19568a;color: white;width: 55%;">
                            <p style="overflow: hidden;text-overflow: ellipsis;white-space:nowrap;line-height: 40px;margin: 0;text-align: center;"><b>报名{{items.codeflagname}} >></b></p>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="item" class="container item-class" style="background: white;height: auto;margin-top: -30px;padding: 10px;margin-bottom: 0;">
            <div id="course_detail_top_right_courseinfo_part_id" style="height: 100%;">
                <div style="padding-top: 10px;text-align: left;"><span style="font-size: 1.1em;">开课时间：{{item.totalbegin}}<br />结束时间：{{item.totalend}}</span></div>
                ...
                <div id="" style="font-size: 1.2em;margin-top: 10px;">
                    <div style="width: 100%;height: auto;padding: 2%;" v-for="item1 in item.courses">
                        <p style=""><b>{{item1.codeflagname}}:</b>{{item1.room}}</p>
                        <p style="text-align: left;">{{item1.time}}</p>
            ...
            <div id="course_detail_bottom_part_id" style="padding: 1em;padding-top: 0;">
    ...
    export default {
        ...
        methods: {
            ...
            getMainData: function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if(_self.CourseList != null && _self.CourseList.length != 0) {
                            _self.item = _self.CourseList[0];
                            _self.period = _self.item.courses[0].period;
                        }
                        _self.hideLoading();
                    },
            ...
            nowEnter: function(index, times) {
                ...
                this.showLoading();
                $.ajax({
                    url: config.DOMIN + "/weixin/jieru/enter",
                    data: {
                        openid: this.openid
                    },
                    type: "post",
                    success: function(data) {
                        ...
                        if(data.flag == 1) {
                            if(data.objectList != null) {
                                var realname = data.objectList.realname ? data.objectList.realname: null;
                                var phone = data.objectList.phone ? data.objectList.phone : null;
                                var summary = data.objectList.summary ? data.objectList.summary : null;
                                var id=index;
                                _self.$router.push({
                                    name: 'WeixinLogin',
                                    params: {
...
<style scoped>
    ...
    .sing-class {
        ...
        border-left: 23px solid #19568a;
        border-top: 23px solid #19568a;
        position: absolute;
    }
